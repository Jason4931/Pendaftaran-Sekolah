...
if (isset($_POST["username"]) && isset($_POST["email"]) && isset($_POST["password"])) {
  $sql = "SELECT * FROM `akun` WHERE `Nama`='$_POST[username]' OR `Email`='$_POST[email]'";
  $result = $conn->query($sql);
  if(mysqli_num_rows($result)==0){
    $sql2 = "INSERT INTO `akun` (`Nama`, `Email`, `Password`) VALUES ('$_POST[username]', '$_POST[email]', '$_POST[password]')";
  ...
  <div class="row">
    ...
    <div class="col-lg-6 col-md-8 col-sm-10 col-12 ml-auto">
      <form method="post" class="card p-4 bg-body-secondary shadow-sm mx-4">
        ...
        <div>
          <input name="username" type="text" class="form-control my-2 py-2" placeholder="Nama" required>
        </div>
        <div>
        ...
        <div class="text-center mt-4">
          ...
          <label><a href="/Pendaftaran Sekolah/?login" style="text-decoration:none">Login</a></label>
        </div>
      </form>
    </div>
    <div class="col-lg-3 col-md-2 col-sm-1 col-0"></div>
